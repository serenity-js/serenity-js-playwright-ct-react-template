import { Ensure, equals } from '@serenity-js/assertions';
import { describe, it } from '@serenity-js/playwright-test';
import { By, PageElement, Value } from '@serenity-js/web';

import type { DropdownOption } from './Dropdown.js';
import { Dropdown as DropdownIO } from './Dropdown.serenity.js';

describe('Dropdown', () => {

    const labels = [
        'First',
        'Second',
        'Third',
    ];

    const options: DropdownOption[] = labels.map(label => ({
        value: label.toLowerCase(),
        label,
    }));

    it('shows the placeholder when no option is selected yet', async ({ story, actor }) => {
        const placeholder = 'Select option';

        const dropdown = story('components/Dropdown/Dropdown/Default', { placeholder, options });

        await actor.attemptsTo(
            Ensure.eventually(
                DropdownIO.placeholder().of(dropdown),
                equals(placeholder),
            ),
        );
    });

    it('shows the available options when the menu is expanded', async ({ story, actor }) => {
        const dropdown = story('components/Dropdown/Dropdown/Default', { options });

        await actor.attemptsTo(
            DropdownIO.open(dropdown),

            Ensure.eventually(
                DropdownIO.availableOptions().of(dropdown),
                equals(labels),
            ),
        );
    });

    it('selects the desired options', async ({ story, actor }) => {
        const dropdown = story('components/Dropdown/Dropdown/Default', { options, allowMultiple: true });

        await actor.attemptsTo(
            DropdownIO.select([
                'First',
                'Third',
            ]).from(dropdown),

            Ensure.that(DropdownIO.selectedOptions().of(dropdown), equals([
                'First',
                'Third',
            ])),
        );
    });

    it('triggers onChange with selected options', async ({ story, actor }) => {
        const dropdown = story('components/Dropdown/Dropdown/WithOnChange', { options, allowMultiple: true });

        await actor.attemptsTo(
            DropdownIO.select([
                'First',
                'Third',
            ]).from(dropdown),

            Ensure.eventually(
                Value.of(PageElement.located(By.css('[data-testid="selected-options"]'))),
                equals(JSON.stringify(['First', 'Third'])),
            ),
        );
    });

    it('allows for selected options to be deselected', async ({ story, actor }) => {
        const dropdown = story('components/Dropdown/Dropdown/Default', { options, allowMultiple: true });

        await actor.attemptsTo(
            DropdownIO.select([
                'First',
                'Third',
                'Second',
            ]).from(dropdown),

            DropdownIO.deselect([
                'First',
            ]).from(dropdown),

            Ensure.that(DropdownIO.selectedOptions().of(dropdown), equals([
                'Third',
                'Second',
            ])),
        );
    });

    it('goes back to showing the placeholder when all the selected options get deselected', async ({ story, actor }) => {
        const placeholder = 'Select option';

        const dropdown = story('components/Dropdown/Dropdown/Default', {
            placeholder,
            options,
            allowMultiple: true,
        });

        await actor.attemptsTo(
            DropdownIO.select([
                'First',
                'Second',
            ]).from(dropdown),

            DropdownIO.deselect([
                'First',
                'Second',
            ]).from(dropdown),

            Ensure.that(DropdownIO.selectedOptions().of(dropdown).length, equals(0)),
            Ensure.that(
                DropdownIO.placeholder().of(dropdown),
                equals(placeholder),
            ),
        );
    });
});
