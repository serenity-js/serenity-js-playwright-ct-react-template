...
[**React**](https://react.dev/) components using [**Serenity/JS**](https://serenity-js.org), [**Playwright Test**](https://playwright.dev/), and the [Serenity/JS `story` fixture](https://serenity-js.org/handbook/test-runners/playwright-test/component-testing/).
It comes pre-configured with **TypeScript**, **Vite**, linting, reporting, and example React components with test scenarios.

Component testing lets you test React components in isolation — without a full application setup.
Vite serves a story gallery that Playwright navigates to render each component under test,
giving you faster feedback, better test isolation, and more focused test scenarios compared to full end-to-end tests.
...
and [Serenity/JS HTML reports](https://serenity-js.org/handbook/reporting/html-reporter/) with trend history, flaky test detection, and an interactive dashboard.

View live reports generated by this template:
- [Serenity/JS HTML Report](https://serenity-js.github.io/serenity-js-playwright-ct-react-template/serenity-js/)
- [Enhanced Playwright Test report](https://serenity-js.github.io/serenity-js-playwright-ct-react-template/playwright/)

## Features

- Pre-configured **Serenity/JS** + **Playwright Test** + **React** + **TypeScript** + **Vite** setup
- Example React components (Dropdown) with corresponding test scenarios
- **Screenplay Pattern** implementation with reusable [interaction objects](https://serenity-js.org/handbook/test-runners/playwright-test/component-testing/) and the `story` fixture
...
Reports are generated automatically during the test run:

- **Serenity/JS HTML Report**: `./reports/serenity-js/index.html`
- **Enhanced Playwright Test Report**: `./reports/playwright/index.html`

To serve the Serenity/JS report locally and open it in your browser:
```
npm run test:report
```
...
npm run test:report     # serves the Serenity/JS report at http://localhost:8080
...
│   │   ├── Dropdown.story.tsx     # Component story (rendered by Vite)
│   │   ├── Dropdown.spec.ts       # Component test scenarios
│   │   ├── Dropdown.serenity.ts   # Serenity/JS interaction object (Screenplay Pattern)
...
playwright/              # Story gallery configuration
├── index.html           # Gallery HTML template served by Vite
└── index.tsx            # Story resolution and mounting logic
...
- `*.story.tsx` — Component stories that define how each component is rendered in isolation
- `*.spec.ts` — Test scenarios using Serenity/JS and the `story` fixture
- `*.serenity.ts` — Reusable interaction objects following the [Screenplay Pattern](https://serenity-js.org/handbook/design/screenplay-pattern/)

### How it works

Each component has a **story file** (`*.story.tsx`) that exports named rendering functions.
Vite serves these stories through a gallery page (`playwright/index.html`).
In your tests, the `story` fixture mounts a component by referencing its story path and returns
an interaction object you use to drive the test through the Screenplay Pattern:
...
describe('Dropdown', () => {
    ...
    it('shows the available options when the menu is expanded', async ({ story, actor }) => {
        ...
        await actor.attemptsTo(
            dropdown.open(),
            Ensure.eventually(dropdown.availableOptions(), equals([ 'First', 'Second', 'Third' ])),
        );
    });
});
```

Learn more in the [Serenity/JS component testing guide](https://serenity-js.org/handbook/test-runners/playwright-test/component-testing/).

## Next steps

- **Replace sample components**: Remove the `Dropdown` example and add your own React components in `src/components/`
- **Add stories**: Create `*.story.tsx` files that export named rendering functions for each component variant
- **Create interaction objects**: Define reusable test code following the Screenplay Pattern (see `Dropdown.serenity.ts` for examples)
- **Write test scenarios**: Add `*.spec.ts` files alongside your components to test them in isolation using the `story` fixture
...
- **Explore the reports**: Review the generated Serenity/JS and Playwright reports to understand test results and identify areas for improvement
...
### Tests failing with "Unknown story"

Ensure your story file exports are correct and that the story path in your test matches the file location relative to `src/`.
For example, a story at `src/components/Dropdown/Dropdown.story.tsx` exporting `Default` is referenced as `'components/Dropdown/Dropdown/Default'`.

## Documentation

- [Serenity/JS Component Testing](https://serenity-js.org/handbook/test-runners/playwright-test/component-testing/)
